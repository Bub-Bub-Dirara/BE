...
def upscale_and_gray(color_bgr: np.ndarray, scale: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    color_bgr = cv2.bilateralFilter(color_bgr, 7, 40, 40)  # 노이즈 완화
...
def ocr_time_near_right(crop_color_bgr: np.ndarray) -> str:
    ...
    pil = Image.fromarray(bin_img)
    data = pytesseract.image_to_string(pil, lang="kor+eng",
                                       config="--oem 1 --psm 7 -c user_defined_dpi=300")
    t = data.replace(' ', '').replace('：', ':')
    m = TIME_RX.search(t)
...
PROFILE_PATTERNS = [
    ...
    # ⚠ '춘식이' 필터는 제거 (실제 대화까지 걸러지는 사례 방지)
]

def looks_spaced_korean(s: str) -> bool:
    # 한글이 6자 이상이고, (한글 공백 한글) 패턴이 2회 이상이면 붙이기
...
def extract_messages(
    ...
    if boxes:
        ...
        # ---- 폴백: 라인 OCR (confidence 평균 포함)
        cfg = "--oem 1 --psm 6 -c user_defined_dpi=300"
        data = pytesseract.image_to_data(
            Image.fromarray(gray), lang="kor+eng", config=cfg,
...
# =========================
def main():
    ap = argparse.ArgumentParser(description="카카오톡/문자 스크린샷 OCR → CSV")
    ...
    messages, meta = extract_messages(
        img_path=args.input,
